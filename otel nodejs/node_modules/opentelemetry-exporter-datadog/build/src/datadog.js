"use strict";
/*
 * Unless explicitly stated otherwise all files in this repository are licensed
 * under the Apache 2.0 license (see LICENSE).
 * This product includes software developed at Datadog (https://www.datadoghq.com/).
 * Copyright 2020 Datadog, Inc.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.DatadogExporter = void 0;
const api = require("@opentelemetry/api");
const core_1 = require("@opentelemetry/core");
const transform_1 = require("./transform");
const types_1 = require("./types");
const url_1 = require("url");
const defaults_1 = require("./defaults");
/**
 * Format and sends span information to Datadog Exporter.
 */
class DatadogExporter {
    constructor(config = {}) {
        this._url =
            config.agentUrl ||
                process.env.DD_TRACE_AGENT_URL ||
                defaults_1.DatadogExportDefaults.AGENT_URL;
        this._logger = config.logger || new api.NoopLogger();
        this._serviceName =
            config.serviceName ||
                process.env.DD_SERVICE ||
                defaults_1.DatadogExportDefaults.SERVICE_NAME;
        this._env = config.env || process.env.DD_ENV;
        this._version = config.version || process.env.DD_VERSION;
        this._tags = config.tags || process.env.DD_TAGS;
        this._flushInterval =
            config.flushInterval || defaults_1.DatadogExportDefaults.FLUSH_INTERVAL;
        this._exporter = new types_1.AgentExporter({ url: new url_1.URL(this._url), flushInterval: this._flushInterval }, new types_1.PrioritySampler());
        this._isShutdown = false;
    }
    /**
     * Called to export sampled {@link ReadableSpan}s.
     * @param spans the list of sampled Spans to be exported.
     */
    export(spans, resultCallback) {
        if (this._isShutdown) {
            setTimeout(() => resultCallback({
                code: core_1.ExportResultCode.FAILED,
                error: new Error('Exporter has been shutdown'),
            }));
            return;
        }
        if (spans.length === 0) {
            return resultCallback({ code: core_1.ExportResultCode.SUCCESS });
        }
        const formattedDatadogSpans = transform_1.translateToDatadog(spans, this._serviceName, this._env, this._version, this._tags);
        try {
            this._exporter.export(formattedDatadogSpans);
            return resultCallback({ code: core_1.ExportResultCode.SUCCESS });
        }
        catch (error) {
            this._logger.debug(error);
            return resultCallback({ code: core_1.ExportResultCode.FAILED });
        }
    }
    /** Stops the exporter. */
    shutdown() {
        this._isShutdown = true;
        return new Promise((resolve, reject) => {
            if (this._exporter._scheduler !== undefined) {
                this._exporter._scheduler.stop();
            }
            resolve();
        });
    }
}
exports.DatadogExporter = DatadogExporter;
//# sourceMappingURL=datadog.js.map